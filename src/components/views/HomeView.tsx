...
interface HomeViewProps {
  ...
  onStartGame: () => void;
  onUpdatePlayersConfig: (players: Omit<Player, 'step' | 'themeIds' | 'sessionPool'>[]) => void;
}
...
export function HomeView({ players, themes, onSelectTheme, onStartGame, onUpdatePlayersConfig }: HomeViewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-start space-y-4 pt-0 px-4 overflow-y-auto no-scrollbar pb-6 relative z-10 w-full">
      <div className="bg-rose-950/40 backdrop-blur-2xl rounded-2xl p-4 border border-white/20 shadow-2xl">
        ...
        <div className="space-y-3 pb-2">
          {players.slice(0, playerCount).map((player) => {
            const selectedThemes = themes.filter(t => player.themeIds.includes(t.id));
            const themeLabel = selectedThemes.length === 0 
              ? '未选择主题 >' 
              : selectedThemes.length === 1 
                ? selectedThemes[0].name 
                : `混合: ${selectedThemes[0].name} +${selectedThemes.length - 1}...`;
            ...
            return (
              <div key={player.id} className="bg-white/5 rounded-xl p-3 border border-white/10 border-l-4 shadow-lg transition-all" style={{borderLeftColor: player.color}}>
                <div className="flex items-center gap-3">
                  ...
                  >
                    <div className="text-[10px] text-white/50 font-bold uppercase tracking-tighter">
                      {selectedThemes.length > 1 ? '多主题混编' : '任务包主题'}
                    </div>
                    <div className={`text-[11px] font-black mt-1 px-2 py-0.5 rounded-md flex items-center gap-1 ${selectedThemes.length > 0 ? 'bg-teal-500/20 text-teal-300' : 'bg-white/5 text-rose-300 animate-pulse border border-rose-300/30'}`}>
                      {themeLabel}
                    </div>
                  </div>
                </div>
